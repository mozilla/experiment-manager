...
check: compose_build_test
	$(COMPOSE_TEST) run app sh -c '$(WAIT_FOR_DB) (${PARALLEL}  "$(ESLINT_CORE)" "$(ESLINT_NIMBUS_UI)" "$(TYPECHECK_NIMBUS_UI)" "$(JS_TEST_CORE)" "$(JS_TEST_NIMBUS_UI)" "$(ESLINT_REPORTING)" "$(TYPECHECK_REPORTING)" "$(JS_TEST_REPORTING)" "$(PYTHON_TEST)") ${COLOR_CHECK}'
